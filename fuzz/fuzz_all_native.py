...
def main():
    parser = argparse.ArgumentParser(
        ...
        description="\n".join([
            "Fuzz helper to run all native go fuzzers in plugin-common",
            "",
        ]),
    )
...
def run_fuzzer(fuzzfn, dir, duration_seconds):
    subprocess.check_call(["go", "test", "-run=^$", f"-fuzz=^{fuzzfn}$", f"-fuzztime={duration_seconds}s", f"github.com/goplugin/plugin-common/pkg/{dir}"], cwd=LIBROOT)
