...
Developing common relayer components such as `chainreader` follows the same pattern as [Core node abstraction Development]. The primary difference is the package location. Instead of adding code to `plugin-common/pkg/loop/internal/core/services` you will work in `plugin-common/pkg/loop/internal/relayer` or `plugin-common/pkg/loop/internal/relayer/pluginprovider` depending on the scope of your changes.
