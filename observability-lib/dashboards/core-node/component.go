	...
	if props.SlackChannel != "" && props.SlackWebhookURL != "" {
		builder.AddContactPoint(grafana.NewContactPoint(&grafana.ContactPointOptions{
			...
			Settings: map[string]interface{}{
				...
				"username":  "Plugin Alerts",
				"title":     `{{ template "slack.plugin.title" . }}`,
				"text":      `{{ template "slack.plugin.text" . }}`,
				"color":     `{{ template "slack.plugin.color" . }}`,
			},
		}))
	}

	notificationPolicyOptions := &grafana.NotificationPolicyOptions{
		Receiver: "plugin-slack",
			...
			Query: []grafana.Query{
				{
					...
					Legend:  "Version: {{version}} https://github.com/goplugin/pluginv3.0/commit/{{commit}} https://github.com/goplugin/pluginv3.0/tree/release/{{version}}",
					Instant: true,
				},
			},
		},
	...
	panels = append(panels, grafana.NewTimeSeriesPanel(&grafana.TimeSeriesPanelOptions{
		PanelOptions: &grafana.PanelOptions{
			...
			AlertOptions: &grafana.AlertOptions{
				...
				RunbookURL:  "https://github.com/goplugin/plugin-common/tree/main/observability-lib",
				For:         "15m",
